...
If ($Install) {
    ...
    <# Cleanup old version - Thanks to Sven Riebe https://github.com/SvenRiebe
    IF (Test-Path 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\{18469ED8-8C36-4CF7-BD43-0FC9B1931AF8}' ) 
    {
        msiexec.exe /x "{18469ED8-8C36-4CF7-BD43-0FC9B1931AF8}" /qn
        Start-Sleep -Seconds 15
    }  # Should be catched by following query #>
    # End of Cleanup old version - Thanks to Sven Riebe https://github.com/SvenRiebe
    $MSIGUIDs = get-wmiobject Win32_Product | Where-Object { $_.Name -like "Dell Power Manager*" }
    IF (! (IsNull($MSIGUIDs) ) )
    {
        Write-Log "Found the following products:
        $($MSIGUID.Name) - $($MSIGUID.IdentifyingNumber)"
        foreach ( $MSIGUID in $MSIGUIDs )
        {  
            Write-Log "Remove MSI: $($MSIGUIDs.Name) - $($MSIGUIDs.IdentifyingNumber) "
            $MSIGUID.Uninstall()
    ...
    get-AppxPackage *DellPower* -AllUsers | Remove-AppxPackage -AllUsers -ErrorAction SilentlyContinue
    Get-AppxProvisionedPackage -Online | Where-Object { $_.DisplayName -like "*dell*Power*" }  | Remove-AppxProvisionedPackage -online -ErrorAction SilentlyContinue
    Start-Sleep -Seconds 15
    

    $exe = "./" + $(Get-ChildItem Dell-Power-Manager-Service*.exe | Sort-Object -Property LastWriteTime -Descending)[0].Name
    $iargs = " /s"
