...
If ($Install) {
    $other_teams = get-wmiobject Win32_Product | ? { $_.Name -Like "Teams*" -and $_.Vendor -Like "Microsoft*" -and -not $_.IdentifyingNumber -like "731F6BAA-A986-45A4-8936-7C3AAAAA760B"}
    IF ( ($other_teams).count -lt 0 ) 
    {  
        Write-Log "Found other msi-based Microsoft Teams installations
        $other_teams"
        #toDo: uninstall other Teams before install
    } 
    Write-Log -Message "Performing Install steps..."
    Start-Process -FilePath "msiexec.exe" -ArgumentList "/i Teams_windows.msi ALLUSERS=1"  -Wait -WindowStyle Hidden -ErrorAction Stop
...
ElseIf ( $UnInstall ) {
    ...
    #Your code goes here
    Start-Process -NoNewWindow -FilePath "msiexec.exe" -ArgumentList ' /x "{731F6BAA-A986-45A4-8936-7C3AAAAA760B}" /qn' -Wait 
