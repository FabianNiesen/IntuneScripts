...
If ($Install) {
    ...
    $MSIGUIDs = get-wmiobject Win32_Product | Where-Object { $_.Name -like "Dell Command*Configure*" }
    IF (! (IsNull($MSIGUIDs) ) )
    {
    ...
    get-AppxPackage *Dell*Command*Configure* -AllUsers | Remove-AppxPackage -AllUsers -ErrorAction SilentlyContinue
    Get-AppxProvisionedPackage -Online | Where-Object { $_.DisplayName -like "*Dell*Command*Configure*" }  | Remove-AppxProvisionedPackage -online -ErrorAction SilentlyContinue

    $exe = "./" + $(Get-ChildItem Dell-Command-Configure*.exe | Sort-Object -Property LastWriteTime -Descending)[0].Name
    $iargs = " /s"
