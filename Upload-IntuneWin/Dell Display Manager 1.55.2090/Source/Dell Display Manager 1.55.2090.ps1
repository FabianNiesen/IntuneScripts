...
.EXAMPLE 
C:\PS> "Dell Display Manager 1.55.2090.ps1" -Install

.EXAMPLE 
C:\PS> "Dell Display Manager 1.55.2090.ps1" -unInstall

.COPYRIGHT
Copyright (c) Microsoft Corporation. All rights reserved. Licensed under the MIT license.
See LICENSE in the project root for license information.

Modified 2022 by Fabian Niesen https://github.com/FabianNiesen
Discalaimer: #Iwork4Dell, but this is a personal project without any relation or support from Dell.
This script is supported as it is, usage is on your own risk. I recommend you check the code carfully 
and test ist before used in production.

.NOTES
Author     : Fabian Niesen
Filename   : Dell Display Manager 1.55.2090.ps1
Requires   : PowerShell Version 3.0
Version    : 1.0
History    : 1.0    FN  23.02.2022  initial version

.LINK
https://github.com/FabianNiesen/IntuneScripts/tree/master/Upload-IntuneWin
...
If ($Install) {
    ...
    # Install DDM - Thanks to Sven Riebe https://github.com/SvenRiebe
    $StopDDMProcess = @() 
    $FolderExist = ""
    Stop-Process -Name ddm -Force -ErrorVariable StopDDMProcess
    If ($StopDDMProcess) 
    { 
        Get-Item -Path 'C:\Program Files (x86)\Dell\Dell Display Manager\unins000.exe' -ErrorVariable FolderExist
        If ($FolderExist) 
        { 
            <# This loop is for Display Manager does not exist on this machine#>
        ...
        Else
        { 
            <# This loop is for Display Manager exist on this machine and DDM is not running#>
            $myApp = Get-ChildItem -Path HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall | Get-ItemProperty | Where-Object {$_.DisplayName -match "Dell Display Manager" } | Select-Object -ExpandProperty QuietUninstallString
            Start-Process cmd.exe -ArgumentList '/c',$myapp -wait -PassThru
            Start-Sleep -Seconds 20
            Start-Process -FilePath ".\ddmsetup.exe" -ArgumentList '/verysilent /noupdate'
            Start-Sleep -Seconds 20
            Stop-Process -Name ddm
    ...
    { 
        <# This loop is for Display Manager exist on this machine and DDM is running#>
        $myApp = Get-ChildItem -Path HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall | Get-ItemProperty | Where-Object {$_.DisplayName -match "Dell Display Manager" } | Select-Object -ExpandProperty QuietUninstallString
        Start-Process cmd.exe -ArgumentList '/c',$myapp -wait -PassThru
        Start-Sleep -Seconds 20
        Start-Process -FilePath ".\ddmsetup.exe" -ArgumentList '/verysilent /noupdate'
    ...
    # End of install DDM - Thanks to Sven Riebe https://github.com/SvenRiebe
...
ElseIf ( $UnInstall ) {
    ...
    # UnInstall DDM - Thanks to Sven Riebe https://github.com/SvenRiebe
    $IDProcess = Get-Process | Where-Object {$_.ProcessName -like 'ddm'} | select -ExpandProperty ID
    Stop-Process -Id $IDProcess -Force
    $myApp = Get-ChildItem -Path HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall | Get-ItemProperty | Where-Object {$_.DisplayName -match "Dell Display Manager" } | Select-Object -ExpandProperty QuietUninstallString
    Start-Process cmd.exe -ArgumentList '/c',$myapp
    Start-Sleep -Seconds 10 
    # End of UnInstall DDM - Thanks to Sven Riebe https://github.com/SvenRiebe
