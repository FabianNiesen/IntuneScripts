...
If ($Install) {
    ...
    $msi = $(Get-ChildItem Setup.RemoteDesktopManagerFree.*.msi | Sort-Object -Property Name -Descending)[0].Name
    Write-Verbose "MSI: $msi"
  
Write-Verbose "Install Base Installer"
Start-Process -NoNewWindow -FilePath "msiexec.exe" -ArgumentList " /i $msi /qn /norestart" -Wait 
...
ElseIf ( $UnInstall ) {
    ...
    #Your code goes here
    Start-Process -NoNewWindow -FilePath "msiexec.exe" -ArgumentList ' /x "{0F549E3C-87C5-43BA-8C14-6241CF1878FC}" /qn' -Wait 
