...
If ($Install) {
    ...
    $MSIGUIDs = get-wmiobject Win32_Product | Where-Object { $_.Name -like "Dell Command | Monitor**" }
    IF (! (IsNull($MSIGUIDs) ) )
    {
    ...
    get-AppxPackage *dell*command*Monitor* -AllUsers | Remove-AppxPackage -AllUsers -ErrorAction SilentlyContinue
    Get-AppxProvisionedPackage -Online | Where-Object { $_.DisplayName -like "*dell*command*Monitor*" }  | Remove-AppxProvisionedPackage -online -ErrorAction SilentlyContinue
    Start-Sleep -Seconds 15
    
    $exe = "./" + $(Get-ChildItem Dell-Command-Monitor*.exe | Sort-Object -Property LastWriteTime -Descending)[0].Name
    $iargs = " /s"
