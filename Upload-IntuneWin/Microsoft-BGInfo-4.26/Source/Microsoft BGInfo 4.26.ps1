...
If ($Install) {
    ...
    New-Item -Path 'REGISTRY::HKEY_USERS\.DEFAULT\Software\Sysinternals\BGInfo' -type Directory -Force -ErrorAction SilentlyContinue
    New-ItemProperty -LiteralPath 'REGISTRY::HKEY_USERS\.DEFAULT\Software\Sysinternals\BGInfo' -Name 'EulaAccepted' -PropertyType DWORD -Value 1 -Force -ErrorAction SilentlyContinue
    New-Item -ItemType Directory -Path $Env:Programfiles\BGInfo -Confirm:$false
    Copy-Item *.* -Destination $Env:Programfiles\BGInfo -Force -Confirm:$false
    Copy-Item .\Run-BGInfo.cmd.lnk -Destination "C:\ProgramData\Microsoft\Windows\Start Menu\Programs\StartUp\" -Force -Confirm:$false
...
ElseIf ( $UnInstall ) {
    ...
    #Your code goes here
    Remove-Item -Path "C:\ProgramData\Microsoft\Windows\Start Menu\Programs\StartUp\Run-BGInfo.cmd.lnk" -Force -Confirm:$false
    Remove-Item -Path "$Env:Programfiles\BGInfo" -Force -Recurse -Confirm:$false
