...
If ($Install) {
    ...
    # Cleanup old installs e.g. from OEM
    ...
    $MSIGUID = $( get-wmiobject Win32_Product | Where-Object { $_.Name -like "Dell* Update for Windows 10" } ).IdentifyingNumber
    IF ((get-wmiobject Win32_Product | Where-Object { $MSIGUID -like -not "{4CCADC13-F3AE-454F-B724-33F6D4E52022}"})) { Start-Process -NoNewWindow -FilePath "msiexec.exe" -ArgumentList " /x $MSIGUID /q" -Wait } 
    #IF ((get-wmiobject Win32_Product | Where-Object { $_.IdentifyingNumber -like "{41D2D254-D869-4CD8-B440-5DF49083C4BA}"})) { Start-Process -NoNewWindow -FilePath "msiexec.exe" -ArgumentList ' /x "{41D2D254-D869-4CD8-B440-5DF49083C4BA}" /q' -Wait } 
    # End Cleanup
    $exe = "./" + $(Get-ChildItem Dell-Command-Update-Application-for-Windows10*.exe | Sort-Object -Property LastWriteTime -Descending)[0].Name
    $iargs = " /s"
...
ElseIf ( $UnInstall ) {
    ...
    $MSIGUID = $( get-wmiobject Win32_Product | Where-Object { $_.Name -like "Dell* Update for Windows 10" } ).IdentifyingNumber
    #$MSIGUID = $( get-wmiobject Win32_Product | ? { $_.Name -match "Dell Update for Windows 10" } ).IdentifyingNumber
    $xargs = " /x " + $MSIGUID + " /q"
    #Your code goes here
    Start-Process -NoNewWindow -FilePath "msiexec.exe" -ArgumentList $xargs -Wait 
